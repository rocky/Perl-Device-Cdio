...
can use this library.
    ...
    $d = Device::Cdio::Device->new(-source=>'/dev/cdrom');
    $d->close();
    $d = Device::Cdio::Device->new(-driver_id=>'$perlcdio::DRIVER_DEVICE');
    $drive_name = $d->get_device();
    $d->eject_media();
    $drc=Device::Cdio::close_tray($drive_name);
    
    @drives = Device::Cdio::get_devices_with_cap($perlcdio::FS_AUDIO);
...
This is an Perl OO interface to the GNU CD Input and Control library
(libcdio) which is written in C. The library encapsulates CD-ROM
reading and control. Perl programs wishing to be oblivious of the OS-
...
For named parameters, each argument anme is preceded by a dash. For
example:
...
with a dash.  If a dash is present in the first argument, we
assume dashes for the subsequent ones.

However the older traditional style of positional parameters is also
supported. So the above can also be written:
