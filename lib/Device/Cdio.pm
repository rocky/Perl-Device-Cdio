...
Control library (C<libcdio>) which is written in C. The library
...
The more object-oriented package L<Device::Cdio> and uses 
perlcdio. (Actually, there are no objects in Device::Cdio, but there
are in its sub packages L<Device::Cdio::Device> and
L<Device::Cdio::Tracks>).

Although perlcdio is perfectly usable on its own, it is expected that
...
complete description of the C library from which this is based, and
