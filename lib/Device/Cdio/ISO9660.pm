...
A hash reference which contains:
...
1 if a directory; '' if a not.

=item tm

A POSIX time structure as often return in unis C<localtime()> or C<gmtime()>.
However we adjust the year to be "normal", e.g. we use 2017 instead of 117
and months start at 1 (for January), not 0. Fields in C<tm> are:
    ...
    yday  # Day in the year (0-365, 1 Jan = 0) */
...
Copyright (C) 2006, 2011, 2017 Rocky Bernstein <rocky@cpan.org>
