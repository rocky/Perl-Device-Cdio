...
ok( defined $dev,
    'Device::Cdio::Device->new(-driver_id=>$perlcdio::DRIVER_DEVICE)' );
my $drive_name = $dev->get_device();
note( 'Device->new(DRIVER_DEVICE)((i.e.:',
    $perlcdio::DRIVER_DEVICE, ')) found: ', $drive_name );
...
if ( $ENV{'CI'} ) {
  ...
  SKIP: {
        ...
        @$nvols[1] = 100;
        @$nvols[3] = 255;
        my $c4 = eq_array( $mvols, $nvols )
          || note( '4 channels are not supported: ', join( ', ', @$mvols ) );
